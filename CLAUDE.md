...
- **Script**: a single self-contained game loop with no external state management:
  ...
  - Pausing (`isPaused`) is toggled by `togglePause()`, bound to both the `#pause-btn` click and the Space key (`event.code === 'Space'`, checked before the movement-key `switch` so it works regardless of pause state; it's a no-op once `isGameOver`). Pausing `clearInterval`s the loop and shows `#pause-overlay`; resuming restarts the interval. Movement keys are ignored while `isPaused` (checked right after the Space handling, before the `switch`), and `init()` resets `isPaused` to `false`.
